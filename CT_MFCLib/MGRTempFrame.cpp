...
IMPLEMENT_DYNCREATE(CMGRTempFrame, CExFrame)
...
/////////////////////////////////////////////////////////////////////////////
// CMGRTempFrame メッセージ ハンドラ

int CMGRTempFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (!toolBar->CreateEx(this, TBSTYLE_FLAT, WS_CHILD | WS_VISIBLE | CBRS_TOP
		...
		!toolBar->LoadToolBar(IDR_MGR_TEMP))
	{
...
// コントラスト設定
//
void CMGRTempFrame::OnMgrContrast() 
{
...
void CMGRTempFrame::OnMgrBack() 
{
	...
	anyData = NULL;		// 領域データは無し．
	CExFrame* frm = pApp->TemplateCallBack(this, APP_CALLBACK_REPL);
	if (frm!=NULL) {
		pApp->TemplateCallBack(this, APP_CALLBACK_MGR);
		DestroyWindow();
	}

	/*
	copy_data_MSGraph(pDoc->msGraph, pApp->callMSGraph);
	if (!pApp->callMSGraph.isNull()) {
		anyData = NULL;		// 領域データは無し．
		CExFrame* frm = pApp->TemplateCallBack(this, APP_CALLBACK_REPL, "");
		pApp->callMSGraph.free();
		if (frm!=NULL) DestroyWindow();
	}
	else {
		CString mesg, noti;
		mesg.LoadString(IDS_STR_FAIL_COPY_FLDATA);
		noti.LoadString(IDS_STR_ERR_MEMORY);
		MessageBox("CMGRTempFrame::OnMgrBack\n\n" + mesg, noti + "?", MB_OK);
	}
	*/
}
